...
בשלב זה של פרויקט מסד הנתונים למערכת ייצור היין וגידול גפנים, התמקדנו בהעמקת ההיגיון העסקי על ידי כתיבת פונקציות, פרוצדורות, טריגרים ותוכניות ראשיות. השלב מדגים יישום של אלמנטים מתקדמים, ביניהם שימוש ב־Cursor ו־RefCursor, לולאות (LOOP), תנאים (IF), טיפול בשגיאות (EXCEPTION) ורשומות (RECORD).

במסגרת זו נכתבה פונקציה המאפשרת למנהל להפיק דוח גמיש ודינמי אודות תהליכי הייצור, לפי טווח תאריכים, סוגי תוצרים וסוגי ציוד. פונקציה נוספת מאפשרת הצגה נוחה של נתוני פריטים בתהליך.

נכתבו גם שתי פרוצדורות: אחת לעדכון אוטומטי של כמויות חומרים בתהליך ייצור חדש, והשנייה לעדכון כמות ציוד שנעשה בו שימוש – שתיהן תומכות ברציפות הפעילות ומונעות טעויות אנוש.

הטריגרים שנכתבו נועדו לוודא שמירה על עקביות ושלמות הנתונים – למשל, טריגר שמוודא כי לא ייווצר תהליך ייצור כאשר אין די חומרים, וטריגר נוסף אחראי לעדכן את מצב הציוד בהתאם למידת השימוש בו בתהליכי ייצור.

בנוסף, יושמו תוכניות ראשיות (Main Programs) לצורך הדגמה ובדיקה של הרכיבים השונים תוך תיעוד פלטים ומקרי קצה.
...
קורסור פנימי: הפונקציה מגדירה ופותחת קורסור שמביא את כל העובדים (worker) שתחילת עבודתם (Year_of_starting_work) קטנה או שווה ל-min_years h- אצלינו זה 2010- כלומר יוצגו בסופו של דבר כל העובדים שהתחילו לעבוד עד 2010 כולל.
