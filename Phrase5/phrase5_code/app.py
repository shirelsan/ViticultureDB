        ...
        if query_type == 'query1':
            ...
            try:
                ...
                cur.execute("""
                    UPDATE worker 
                    SET role = CASE 
                        WHEN (%s - Year_of_starting_work) < 5 THEN 'Junior'
                        WHEN (%s - Year_of_starting_work) >= 5 AND (%s - Year_of_starting_work) <= 15 THEN 'Mid-level'
                        ELSE 'Senior'
                    END
                    WHERE Year_of_starting_work <= %s
                """, (year_param, year_param, year_param, year_param))

                cur.execute("""
                    SELECT worker_id, w_name, Year_of_starting_work, 
                           (%s - Year_of_starting_work) AS experience_years, role
                    FROM worker
                    WHERE Year_of_starting_work <= %s
                    ORDER BY worker_id
