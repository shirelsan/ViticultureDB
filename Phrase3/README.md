...
הסבר על המבט:המבט maintenance_task_info נועד להציג מידע מרוכז עבור משימות תחזוקה שהתבצעו בכרמים.
המבט מאגד מידע ממספר טבלאות:

maintenace – שמכילה פרטי משימות (סוג המשימה, תאריך),

worker – שממנה נמשכת זהות העובד שביצע את המשימה,

Receive – שמחברת בין משימה לבין כרם מסוים,

vineyard – שמספקת מידע נוסף על הכרם עצמו (מיקום, סוג קרקע).
...
Q1
שאילתה זו מציגה את מספר משימות התחזוקה שבוצעו בכל סוג קרקע.
...
Q2
שאילתה זו מזהה עבור כל כרם את העובד שביצע בו את מספר משימות התחזוקה הגבוה ביותר.
